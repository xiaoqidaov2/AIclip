...
class QueryOrchestrator:
    ...
    def plan(self, user_input: str, locked_skill: Optional[str] = None) -> OrchestrationResult:
        ...
        if plan_payload is not None:
            steps = plan_to_steps(user_input, route, plan_payload, self.tool_setup)
            return OrchestrationResult(route=route, steps=steps, nudges=build_nudges(steps), planner_mode="llm", raw_plan=plan_payload)
        skills = discover_skills(route, self.tool_setup)
        steps = build_steps(user_input, skills, route, self.tool_setup)
