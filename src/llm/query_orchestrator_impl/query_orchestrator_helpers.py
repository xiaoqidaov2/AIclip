...
def discover_skills(user_input: str, route: SkillDecision, tool_setup: Any) -> List[str]:
    all_skill_names = tool_setup.get_skill_names()
    if route.skill_name not in all_skill_names:
        return []
    candidate_text = user_input.lower()
    matched: List[str] = [route.skill_name]
    for skill_name in all_skill_names:
        if skill_name == route.skill_name:
            continue
        skill = tool_setup.get_skill(skill_name)
        if any(kw and kw.lower() in candidate_text for kw in skill.keywords):
...
def build_steps(
    ...
    for idx, skill_name in enumerate(skills):
        if idx == 0:
            ...
            prompt = f"Continue processing using {skill_name} tools for aspects not covered by previous steps."
        steps.append(
            PlannedStep(
                phase=OrchestrationPhase.EXECUTE,
                skill_name=skill_name,
                reason=route.reason if idx == 0 else f"heuristic: keyword match for {skill_name}",
                prompt=prompt,
                input_text=user_input,
                locked=route.locked,
                allowed_tools=list(tool_setup.get_skill(skill_name).tool_names),
                stop_conditions=["Use only the active skill tools."],
            )
        )
    if not steps:
        fallback_skill = route.skill_name if route.skill_name in tool_setup.get_skill_names() else tool_setup.get_skill().name
        steps.append(
            PlannedStep(
                phase=OrchestrationPhase.EXECUTE,
                skill_name=fallback_skill,
                reason=route.reason or "fallback skill",
                prompt="Handle the request with the available tools.",
                input_text=user_input,
                locked=route.locked,
                allowed_tools=list(tool_setup.get_skill(fallback_skill).tool_names),
                stop_conditions=["Use only the active skill tools."],
            )
        )
    return steps
    ...
    for raw in plan.get("steps") or []:
        ...
        steps.append(
            PlannedStep(
                ...
                metadata={"role": raw.get("role"), "raw_phase": phase_name},
            )
        )
    if not steps:
        return build_steps(user_input, discover_skills(user_input, route, tool_setup), route, tool_setup, session_context=session_context)
    if all(step.skill_name != route.skill_name for step in steps):
        steps.insert(
            ...
            PlannedStep(
                ...
                input_text=user_input,
                locked=route.locked,
                allowed_tools=list(tool_setup.get_skill(route.skill_name).tool_names),
...
def build_nudges(steps: List[PlannedStep]) -> List[str]:
    if len(steps) > 1:
        return ["Prefer sequential loading per stage: discover -> execute -> compress."]
    return []
