...
def build_system_prompt(
    ...
    prompt += (
        ...
        + "Read `ok`, `code`, `status` for success/failure context.\n"
        + "Read `next_actions` for suggested next steps (informational, not auto-executed).\n"
        + "Read `state` for project state when present; read `payload` for structured data.\n"
        + "On failure, `validation` and `render_state` carry error/blocker details.\n"
        + "Treat `message` as display text. `summary` appears only when it differs from `message`.\n"
        ...
        + "When applying multiple overlays or adding multiple clips/assets, prefer sequential tool calls over parallel ones to avoid state conflicts.\n"
        + "For `generate_animejs_overlay_asset`, the `code` parameter is analyzed to extract colors and animation style. Use explicit color values (hex like `#FF6B35` or rgb like `rgb(255,100,50)`) in your code so the rendered overlay matches your intent.\n"
        + "For `generate_animejs_overlay_asset`, the `label` must accurately describe the visual effect actually implemented by the `code`. Do not generate a label that describes shapes, colors, or animations that the code does not produce. Always verify that the label matches the code before calling the tool.\n"
        + "For `apply_overlay_to_screen`, use normalized coordinates (0.0-1.0) for `x` and `y` — e.g., `x=0.5` places the overlay at the horizontal center. The overlay is centered on that anchor point.\n"
        + "When placing overlays, check `detect_faces` results for `face_zones` and avoid positioning overlays over detected face areas.\n"
        + "After transcription, review subtitle text for obvious ASR errors (misheard words, wrong characters, nonsensical phrases) and fix them with `update_project_subtitle` before proceeding.\n"
