...
class ProjectToolPostParseUpdateProjectSubtitleMixin:
    ...
    def update_project_subtitle(
        ...
        with self.store.project_lock(project_path):
            ...
            command = UpdateSubtitleCueCommand(
                ...
                text=(
                    self._normalize_transcribed_text(text) if text is not None else None
                ),
                spans=(
                    ...
                    if normalized_spans is not None
                    else None
                ),
                speaker=speaker,
                language=language,
