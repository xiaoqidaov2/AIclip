...
class ProjectToolPostParseDetectFacesMixin:
    ...
    def detect_faces(
        ...
        with self.store.project_lock(project_path):
            ...
            # Compute face zone summary to help overlay placement avoid faces
            all_norm_x = []
            all_norm_y = []
            all_norm_w = []
            all_norm_h = []
            for fr in frame_results:
                for face in fr.get("faces", []):
                    all_norm_x.append(face.get("norm_x", 0))
                    all_norm_y.append(face.get("norm_y", 0))
                    all_norm_w.append(face.get("norm_w", 0))
                    all_norm_h.append(face.get("norm_h", 0))

            face_zones: Dict[str, Any] = {}
            if all_norm_x:
                min_x = max(0.0, min(all_norm_x) - 0.05)
                max_x = min(1.0, max(all_norm_x) + max(all_norm_w) + 0.05)
                min_y = max(0.0, min(all_norm_y) - 0.05)
                max_y = min(1.0, max(all_norm_y) + max(all_norm_h) + 0.05)
                face_zones = {
                    "x_range": [round(min_x, 3), round(max_x, 3)],
                    "y_range": [round(min_y, 3), round(max_y, 3)],
                    "description": (
                        f"Face area spans x={min_x:.2f}-{max_x:.2f}, y={min_y:.2f}-{max_y:.2f}. "
                        f"Place overlays outside this region to avoid遮挡."
                    ),
                }
            else:
                face_zones = {"description": "No faces detected. Overlays can be placed anywhere."}

            project.metadata["detected_faces"] = frame_results
            project.metadata["face_zones"] = face_zones
            ...
            return ToolResult(
                ...
                summary=f"Found faces in {frames_with_faces}/{len(frame_results)} frames, {total_faces} total detections. {face_zones.get('description', '')}",
            ).to_dict()
