...
class ProjectToolPostParseCreateHelpersMixin:
    ...
    def _existing_project_response(self, existing_project: Project, existing_project_path: Path, source_path: Path) -> Dict[str, Any]:
        ...
        return ToolResult(
            ...
            state={"project_path": str(existing_project_path), "media_path": str(source_path), "reused_existing_project": True, **self._project_counts(existing_project)},
            payload=existing_project.to_dict(),
            summary=f"Reused existing project {existing_project.name}",
        ).to_dict()

    def _build_project_from_media(self, source_path: Path, media_info: Dict[str, Any], project_name: Optional[str], project_id: Optional[str], asset_id: Optional[str], track_id: str, clip_id: str) -> Project:
        asset = Asset(
            ...
            metadata={"source_media_path": str(source_path), "has_audio": media_info["has_audio"], "fps": media_info["fps"], "size": media_info["size"], "media_kind": media_info.get("media_kind", "video")},
        )
        clip_duration = media_info["duration"] if media_info["duration"] > 0 else 0.0
        track_kind = media_info.get("media_kind", "video")
        tracks = [Track(id=track_id, kind=track_kind, name="Main Audio" if track_kind == "audio" else "Main Video", clips=[Clip(id=clip_id, asset_id=asset.id, start=0.0, end=clip_duration, source_in=0.0, source_out=clip_duration)])]
        if track_kind == "video" and media_info["has_audio"]:
            tracks.append(Track(id="audio_1", kind="audio", name="Main Audio", role="dialogue", volume=1.0, clips=[Clip(id=f"{clip_id}_audio", asset_id=asset.id, start=0.0, end=clip_duration, source_in=0.0, source_out=clip_duration)]))
        return Project(
            id=project_id or source_path.stem,
            name=project_name or source_path.stem,
            assets=[asset],
            timeline=Timeline(duration=clip_duration, fps=media_info["fps"] or None, tracks=tracks),
            metadata={
    ...
    def _created_project_response(self, project: Project, report: Any, saved_path: Path, source_path: Path, media_info: Dict[str, Any]) -> Dict[str, Any]:
        return ToolResult(
            ok=report.passed,
            status="ok" if report.passed else "warn",
            ...
            state={"project_path": str(saved_path), "media_path": str(source_path), "subtitle_source_present": False, "audio_track_present": media_info["has_audio"], **self._project_counts(project)},
