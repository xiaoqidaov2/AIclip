...
def normalize_tool_result(operation: str, raw: Any, *, entity: Optional[str] = None, project_id: Optional[str] = None, project_version: Optional[int] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and {"status", "code", "message"}.issubset(raw.keys()):
        ...
        result_dict.setdefault("decision", {"operation": result_dict["operation"], "code": result_dict.get("code"), "status": result_dict.get("status"), "ok": result_dict.get("ok", str(result_dict.get("status", "")).lower() not in {"error", "failed", "fail"}), "project_id": result_dict.get("project_id"), "project_version": result_dict.get("project_version"), "validation": result_dict.get("validation"), "render_state": result_dict.get("render_state"), "state": result_dict.get("state", {}), "next_actions": ensure_list(result_dict.get("next_actions"))})
    ...
    if isinstance(raw, dict):
        ...
        tool_result.decision = {"operation": operation, "code": tool_result.code, "status": tool_result.status, "ok": tool_result.ok, "project_id": tool_result.project_id, "project_version": tool_result.project_version, "validation": asdict(tool_result.validation), "render_state": asdict(tool_result.render_state), "state": tool_result.state, "next_actions": list(tool_result.next_actions)}
    ...
    tool_result.decision = {"operation": operation, "code": tool_result.code, "status": tool_result.status, "ok": tool_result.ok, "project_id": tool_result.project_id, "project_version": tool_result.project_version, "validation": asdict(tool_result.validation), "render_state": asdict(tool_result.render_state), "state": tool_result.state, "next_actions": []}
    return tool_result.to_dict()
