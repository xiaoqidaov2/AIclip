...
class ProjectStore:
    ...
    def save(
        ...
        *,
        ...
        target_format = format or (
            "xml" if file_path.suffix.lower() == ".xml" else "json"
        )
    ...
    def mutate(
        ...
        *,
        ...
        if source_path == target_path:
            ...
            with self.project_lock(source_path):
                ...
                saved_path = self.save(
                    project, target_path, format=format, indent=indent, _already_locked=True
                )

        else:

            # Acquire both locks in a consistent order (by normalised path string) to
            # prevent deadlocks when two threads swap source/target roles simultaneously.

            first_path, second_path = sorted([source_path, target_path], key=str)

            with self.project_lock(first_path):

                with self.project_lock(second_path):
    ...
    def resolve_asset_path(
        self, asset_path: str | Path, project_path: str | Path
    ) -> Path:
    ...
    def find_or_create_project_for_media(
        ...
        workspace, created = self.registry.find_or_create(
            media_path, project_name=project_name
        )

        return workspace.project_file, created

    def register_media_project(
        self, media_path: str | Path, project_path: str | Path
    ) -> None:
