...
def test_tool_result_to_dict_populates_summary_and_decision_defaults():
    ...
    assert result["decision"]["operation"] == "demo"
    assert result["decision"]["validation"]["warnings"] == ["w"]


def test_tool_result_to_dict_merges_payload_mapping():
    payload = {"output_path": "out.mp4", "message": "custom"}
    result = ToolResult(ok=True, status="ok", code="x.ok", message="base", operation="x", payload=payload).to_dict()
...
def test_normalize_tool_result_for_already_normalized_dict_infers_ok():
    ...
    assert normalized["decision"]["status"] == "failed"
...
def test_normalize_tool_result_for_generic_dict_normalizes_nested_fields():
    ...
    assert normalized["decision"]["ok"] is True
...
def test_normalize_tool_result_for_scalar_payload():
    normalized = normalize_tool_result("describe", "plain text", project_id="p1", project_version=3)
    assert normalized["ok"] is True
    assert normalized["content"] == "plain text"
    assert normalized["decision"]["project_id"] == "p1"
    assert normalized["decision"]["project_version"] == 3


def test_serialize_tool_result_returns_json_string():
    output = serialize_tool_result("op", {"status": "ok", "code": "op.ok", "message": "m"})
