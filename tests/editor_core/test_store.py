...
def test_mutate_updates_project_and_can_write_to_output_path(tmp_path):
    ...
    project, result, saved_path = store.mutate(
        source_path, mutator, output_path=output_path
    )
...
def test_register_and_find_project_for_media(tmp_path):
    ...
    project_path, created = store.find_or_create_project_for_media(
        media_path, project_name="Store Registry"
    )
    ...
    project_path.write_text(
        json.dumps({"id": "project-1", "name": "Store Test"}), encoding="utf-8"
    )
...
def test_import_asset_to_project_copies_external_media_into_workspace(tmp_path):
    store = ProjectStore()
    media_source = tmp_path / "source.mp4"
    project_path = tmp_path / "projects" / "demo" / "project.json"

    media_source.write_bytes(b"video-bytes")

    imported = store.import_asset_to_project(media_source, project_path)

    assert imported == "media/source.mp4"
    assert (project_path.parent / "media" / "source.mp4").read_bytes() == b"video-bytes"


def test_normalize_project_asset_paths_imports_absolute_media(tmp_path):
    store = ProjectStore()
    media_source = tmp_path / "source.mp4"
    project_path = tmp_path / "projects" / "demo" / "project.json"
    project = _sample_project()
    ...
    media_source.write_bytes(b"video-bytes")
    project.assets[0].path = str(media_source.resolve())

    changed = store.normalize_project_asset_paths(project, project_path)

    assert changed is True
    assert project.assets[0].path == "media/source.mp4"
    assert (project_path.parent / "media" / "source.mp4").exists()
