...
def add_captions(
    ...
    try:
        ...
        # 2. 验证字幕信息
        caption_items = validate_captions_data(captions)
...
async def add_captions_async(
    ...
    Args:
        ...
        captions: 字幕信息列表，详见 add_captions 函数
        text_color: 文本颜色（十六进制），默认"#ffffff"
        border_color: 边框颜色（十六进制），默认 None
        alignment: 文本对齐方式（0-5），默认 1
        alpha: 文本透明度（0.0-1.0），默认 1.0
...
def validate_captions_data(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    验证字幕数据列表，处理可选字段的默认值
