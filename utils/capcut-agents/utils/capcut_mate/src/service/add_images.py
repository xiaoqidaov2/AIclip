...
def _prepare_images_local_files(draft_url: str, image_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
async def add_images_async(
    ...
    Args:
        ...
        image_infos: 图片信息列表，详见 add_images 函数
...
def parse_image_data(image_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    校验并规范化图片数据，处理可选字段的默认值
    ...
    """
    logger.info(f"Processing {len(image_infos)} image items")
