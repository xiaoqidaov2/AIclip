...
def _prepare_audios_local_files(draft_url: str, audio_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
def _add_audios_internal(
    ...
    logger.info(f"add_audios, draft_url: {draft_url}, audio_infos count: {len(audio_infos)}")
...
async def add_audios_async(
    ...
    Args:
        ...
        audio_infos: 音频信息列表，详见 add_audios 函数
...
def parse_audio_data(audio_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    校验并规范化音频数据，处理可选字段的默认值
    ...
    """
    # 验证数据格式
    validate_input_format(audio_infos)
    
    # 处理音频项列表
    result = []
    for i, item in enumerate(audio_infos):
