...
def add_effects(
    ...
    """
    logger.info(f"add_effects started, draft_url: {draft_url}, effects count: {len(effect_infos) if effect_infos else 0}")
    ...
    # 2. 验证特效信息
    effect_items = validate_effects_data(effect_infos)
...
async def add_effects_async(
    ...
    Args:
        ...
        effect_infos: 特效信息列表，详见 add_effects 函数
...
def validate_effects_data(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    验证特效数据列表，验证必选字段和数值范围
