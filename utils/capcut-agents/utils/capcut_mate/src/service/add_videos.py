...
def _prepare_videos_local_files(draft_url: str, video_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
async def add_videos_async(
    ...
    if isinstance(video_infos, str):
        parsed_video_infos = json.loads(video_infos)
    else:
        parsed_video_infos = video_infos
    prepared_videos = _prepare_videos_local_files(
        draft_url=draft_url,
        video_infos=parsed_video_infos,
    )
...
def _add_videos_internal(
    ...
    Args:
        draft_url: 草稿 URL
        video_infos: 视频信息列表（当 prepared_videos 为 None 时参与解析）
    ...
    # 7. 保存草稿
    logger.info(f"Before save: script.duration={script.duration}, track_name={track_name}")
    script.save()
    ...
    if os.path.exists(draft_content_path):
        ...
        logger.info(f"After save: draft_content.json duration={saved_content.get('duration', 'NOT_FOUND')}")
...
def add_video_to_draft(
    ...
    try:
        ...
        # 检查视频素材时长是否正确获取
        logger.info(f"Video material loaded: path={video_path}, duration={video_material.duration}, width={video_material.width}, height={video_material.height}")
        if video_material.duration <= 0:
            logger.warning(f"Video material duration is 0 or negative: {video_material.duration}, will use display_duration as fallback")
        ...
        # 确保 video_material.duration 有效，如果为0则使用 display_duration 作为备选
        material_duration = video_material.duration if video_material.duration > 0 else display_duration
        source_duration = min(material_duration, display_duration)
        logger.info(f"Video segment timerange: material_duration={material_duration}, source_duration={source_duration}, display_duration={display_duration}")
...
def parse_video_data(video_infos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    校验并规范化视频数据，处理可选字段的默认值
    
    Args:
        video_infos: 视频信息列表，格式如下：
        [ 
            {
    ...
    Raises:
        CustomException: 当数据格式错误或缺少必选字段时抛出
    """
    # 确保输入是列表
    ...
    for i, item in enumerate(video_infos):
