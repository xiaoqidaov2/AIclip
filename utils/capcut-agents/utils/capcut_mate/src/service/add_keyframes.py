...
def add_keyframes(
    ...
    # 2. 验证关键帧信息
    keyframe_items = validate_keyframes_data(keyframes)
...
async def add_keyframes_async(
    ...
    Args:
        ...
        keyframes: 关键帧信息列表，详见 add_keyframes 函数
...
def validate_keyframes_data(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    验证关键帧数据列表，验证必选字段和数值范围
    
    Args:
        data: 包含关键帧数据的列表，格式如下：
        [
            {
