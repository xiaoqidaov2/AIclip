...
# 进程启动时加载花字数据到内存（懒加载）
_HUAZI_DATA: Optional[List[Dict[str, Any]]] = None
_HUAZI_MAP_BY_NAME: Optional[Dict[str, Dict[str, Any]]] = None
_HUAZI_MAP_BY_ID: Optional[Dict[str, Dict[str, Any]]] = None
...
def _ensure_huazi_loaded() -> None:
    """确保花字数据已加载（懒加载）"""
    global _HUAZI_DATA
    if _HUAZI_DATA is None:
        _load_huazi_data()
