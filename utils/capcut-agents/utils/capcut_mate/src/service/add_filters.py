...
def add_filters(
    ...
    """
    logger.info(f"add_filters started, draft_url: {draft_url}, filters count: {len(filter_infos) if filter_infos else 0}")
    ...
    # 2. 验证滤镜信息
    filter_items = validate_filters_data(filter_infos)
...
async def add_filters_async(
    ...
    Args:
        ...
        filter_infos: 滤镜信息列表，详见 add_filters 函数
...
def validate_filters_data(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    验证滤镜数据列表，验证必选字段和数值范围
