...
def download(url: str, save_dir: str, limit: int = DEFAULT_FILE_SIZE_LIMIT,
    ...
    """
    local_path = _resolve_local_source(url)
...
def _resolve_local_source(source: str) -> Optional[str]:
    """识别并返回本地文件路径；远程 URL 返回 None。"""
    if not source:
        return None
    ...
    if parsed.scheme == "file":
        local_path = _file_uri_to_path(parsed)
    elif _looks_like_windows_path(stripped) or stripped.startswith("/") or stripped.startswith("\\"):
        local_path = stripped

    if local_path is None:
        return None

    normalized_path = os.path.abspath(os.path.normpath(local_path))
    if not os.path.isfile(normalized_path):
        logger.warning(f"Local file not found: {normalized_path}")
        raise CustomException(CustomError.DOWNLOAD_FILE_FAILED, f"Local file not found: {normalized_path}")

    logger.info(f"Using local file directly: {normalized_path}")
...
def _file_uri_to_path(parsed) -> str:
    path = url2pathname(unquote(parsed.path or ""))
    if parsed.netloc and parsed.netloc not in ("", "localhost"):
        netloc = url2pathname(unquote(parsed.netloc))
        if path:
