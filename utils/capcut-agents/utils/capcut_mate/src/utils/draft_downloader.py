...
def download_single_file(file_url: str, target_dir: str) -> bool:
    ...
    while retry_count <= max_retries:
        try:
            ...
            # 对于JSON文件，检查响应内容是否为错误JSON（HTTP 200陷阱）
            if file_url.endswith('.json'):
                try:
                    json_data = response.json()
                    # 检查是否为错误响应（包含code和message字段）
                    if isinstance(json_data, dict) and 'code' in json_data and json_data.get('code') != 0:
                        error_msg = json_data.get('message', '未知错误')
                        logger.error(f"下载JSON文件返回错误响应: code={json_data.get('code')}, message={error_msg}, URL: {file_url}")
                        return False
                except json.JSONDecodeError:
                    # 不是JSON格式，继续正常处理（可能是二进制或其他格式）
                    pass
...
def update_json_file_paths(json_file_path: str, target_dir: str, draft_id: str) -> bool:
    ...
    try:
        ...
        # 检查是否为错误响应JSON（HTTP 200陷阱）
        if isinstance(data, dict) and 'code' in data and data.get('code') != 0:
            error_msg = data.get('message', '未知错误')
            logger.error(f"JSON文件内容是错误响应: code={data.get('code')}, message={error_msg}, 文件: {json_file_path}")
            return False
