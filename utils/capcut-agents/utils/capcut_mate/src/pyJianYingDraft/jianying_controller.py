...
class JianyingController:
    ...
    def find_and_click_draft(self, draft_name: str, max_retries: int = 10, retry_interval: float = 3.0) -> None:
        ...
        for attempt in range(max_retries):
            try:
                # 尝试触发 UI 刷新：移动到窗口中心并轻微滚动
                if attempt > 0:
                    logger.info(f"正在尝试刷新剪映列表 (第 {attempt} 次)...")
                    rect = self.app.BoundingRectangle
                    center_x, center_y = (rect.left + rect.right) // 2, (rect.top + rect.bottom) // 2
                    pyautogui.moveTo(center_x, center_y)
                    pyautogui.scroll(-100) # 向下滚动一点
                    time.sleep(1)
                    pyautogui.scroll(100)  # 滚回来
                    time.sleep(1)
                ...
                # 方法1: 使用 desc_matcher 查找（兼容原有逻辑）
                draft_name_text = self.app.TextControl(
                    searchDepth=15,
                    Compare=lambda ctrl, depth: ctrl.GetPropertyValue(30159) == target_desc if hasattr(ctrl, 'GetPropertyValue') else False
                )
                
                # 方法2: 如果方法1失败，使用递归查找
                if not draft_name_text.Exists(0.5):
                    logger.info(f"方法1未找到，尝试递归查找: {target_desc}")
                    found_control = None
                    
                    def find_draft_control(control, depth=0):
                        nonlocal found_control
                        if found_control is not None or depth > 15:
                            return
                        try:
                            for child in control.GetChildren():
                                try:
                                    if hasattr(child, 'GetPropertyValue'):
                                        desc = child.GetPropertyValue(30159)
                                        if desc == target_desc:
                                            found_control = child
                                            return
                                except:
                                    pass
                                find_draft_control(child, depth + 1)
                        except:
                            pass
                    
                    find_draft_control(self.app)
                    
                    if found_control is not None:
                        draft_name_text = found_control
                        logger.info(f"递归查找成功找到草稿控件")
                    else:
                        raise exceptions.DraftNotFound(f"未找到名为{draft_name}的剪映草稿")
                
                draft_btn = draft_name_text.GetParentControl()
                if draft_btn is None:
                    raise AutomationError(f"无法获取草稿 {draft_name} 的父控件")
                
                # 获取控件中心点并使用 pyautogui 点击，解决缩放导致的偏移问题
                rect = draft_btn.BoundingRectangle
                x, y = (rect.left + rect.right) // 2, (rect.top + rect.bottom) // 2
                logger.info(f"找到草稿，点击坐标: ({x}, {y})")
                pyautogui.click(x, y)
                ...
                last_exception = e
                # 特别处理 COMError 或 ElementNotFound
                error_str = str(e)
                if "COMError" in error_str or "订户" in error_str:
                    logger.warning(f"检测到 COM 通讯异常: {error_str}，尝试重新获取窗口...")
                    time.sleep(2)
                    try:
                        self.get_window()
                    except:
                ...
                if attempt < max_retries - 1:
                    logger.info(f"未找到草稿或操作失败，第{attempt + 1}次重试: {error_str}")
                    time.sleep(retry_interval)
        
        # 所有重试都失败，抛出异常
        if isinstance(last_exception, exceptions.DraftNotFound):
            raise last_exception
        raise AutomationError(f"查找或点击草稿失败: {last_exception}")
